...
namespace Glue.Data.Providers.MySql
{
    ...
    public class MySqlMappingProviderOBSOLETE : MySqlDataProvider, IMappingProvider
    {
        ...
        public object Find(Type type, params object[] keys)
        {
            ...
            MySqlCommand command = CreateCommand(info.FindCommandText);
            int i = 0;
        ...
        {
            Entity info = Obtain(type);
            using (IDataReader reader = ExecuteReader((MySqlCommand)command))
            {
        ...
        public Array ListManyToMany(object left, Type right, string jointable, Filter filter, Order order, Limit limit)
        {
            ...
            MySqlCommand command = CreateCommand(join.ToString());
        ...
        public void AddManyToMany(object left, object right, string jointable)
        {
            ...
            QueryBuilder s = CreateQueryBuilder();
            s.Append(" REPLACE INTO ").Identifier(info.JoinTable);
            s.Append(" SET ");
            s.Identifier(info.JoinLeftKey).Append("=").Parameter(info.JoinLeftKey).Append(",");
            s.Identifier(info.JoinRightKey).Append("=").Parameter(info.JoinRightKey);
            
            MySqlCommand command = CreateCommand(s.ToString());
            AddParameter(command, info.JoinLeftKey, info.LeftKeyInfo.GetValue(left));
